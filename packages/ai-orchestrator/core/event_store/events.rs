...
/// Helper macro to extract correlation_id from any enum variant that
/// carries a `correlation_id: Option<Uuid>` field. Removes N match
/// arms per event enum (was 7 for ResearchWorkflowEvent, 4 for
/// AIAgentEvent) and keeps the pattern DRY as new variants are added.
...
impl DomainEvent for ResearchWorkflowEvent {
    ...
    impl_correlation_id!(
        ...
        WorkflowCreated, ExecutionStarted, TaskCreated,
        TaskCompleted, ExecutionCompleted, ExecutionFailed, WorkflowUpdated
    );
...
impl DomainEvent for AIAgentEvent {
    ...
    impl_correlation_id!(
        AIAgentEvent,
        AgentCreated, TaskAssigned, ResponseGenerated, AgentError
    );
