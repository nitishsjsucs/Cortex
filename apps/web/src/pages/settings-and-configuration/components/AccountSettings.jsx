...
const ACCT_KEY = 'cortex_account_settings';

const AccountSettings = () => {
  const saved = (() => { try { return JSON.parse(localStorage.getItem(ACCT_KEY)) || {}; } catch { return {}; } })();

  const [profileData, setProfileData] = useState(saved.profileData || {
  ...
  const [twoFactorEnabled, setTwoFactorEnabled] = useState(saved.twoFactorEnabled ?? true);
  const [emailNotifications, setEmailNotifications] = useState(saved.emailNotifications ?? true);

  // Persist to localStorage whenever values change
  useEffect(() => {
    try {
      localStorage.setItem(ACCT_KEY, JSON.stringify({ profileData, twoFactorEnabled, emailNotifications }));
    } catch { /* ignore */ }
  }, [profileData, twoFactorEnabled, emailNotifications]);
  ...
  const handleSaveProfile = () => {
    // Values auto-save via useEffect; show confirmation
